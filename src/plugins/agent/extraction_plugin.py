...
class ExtractionPlugin:
    ...
    @kernel_function(
        description="Detect conflicts and contradictions in a change request to prevent incorrect updates",
    ...
    async def detect_conflicts(
        ...
        """
        Analyze change request for conflicts and contradictions.

        Checks for:
        - Internal contradictions in the request
        - Multiple conflicting values for the same field
        - Ambiguous instructions
        - Suspicious patterns

        Args:
            extracted_data: JSON string of extracted change request data
        ...
        try:
            ...
            conflict_prompt = f"""
Analyze this SRM change request for conflicts, contradictions, or ambiguity.
...
Check for:
1. CONTRADICTIONS: Does the request contain conflicting information?
   - Example: "Mark as approved" but also "pending review"
   - Example: Two different values requested for the same field

2. AMBIGUITY: Is the request unclear about what should be done?
   - Example: "Update notes" without specifying what to write
   - Example: Multiple SRMs mentioned without clarity on which to update

3. SUSPICIOUS PATTERNS:
   - Sender claiming authority they may not have
   - Urgency tactics without proper justification
   - Request to hide or remove important information

4. FIELD CONFLICTS:
   - Requesting updates to both owner_notes AND hidden_notes with conflicting information
   - Recommendation logic contradicting exclusion criteria

Respond ONLY with a JSON object (no markdown, no extra text):
{{
    "has_conflicts": true/false,
    "conflict_details": "Clear description of the conflict(s) found, or 'No conflicts detected' if none",
    "severity": "low/medium/high",
    "safe_to_proceed": true/false,
    "conflicts": ["list", "of", "specific", "issues"]
}}

Rules:
- If ANY contradictions found: has_conflicts=true, safe_to_proceed=false
- If request is clear and consistent: has_conflicts=false, safe_to_proceed=true
- severity="high" for direct contradictions, "medium" for ambiguity, "low" for minor concerns
...
"""
            ...
            async def _detect():
                ...
                chat_history.add_system_message(
                    "You are a conflict detection system. Analyze requests for contradictions "
                    "and ambiguity. Always respond with valid JSON only, no markdown formatting."
                )
                chat_history.add_user_message(conflict_prompt)
            ...
            required_fields = ["has_conflicts", "conflict_details", "severity", "safe_to_proceed"]
            for field in required_fields:
                if field not in parsed_result:
                    parsed_result[field] = False if field == "has_conflicts" or field == "safe_to_proceed" else "unknown"

            # Ensure conflicts list exists
            if "conflicts" not in parsed_result:
                parsed_result["conflicts"] = []

            return json.dumps(parsed_result, indent=2)

        except (json.JSONDecodeError, ValueError, TypeError) as e:
            self.logger.error(f"Error parsing conflict detection result: {e}")

            # Return safe default: flag as needing review
            fallback_result = {
                "has_conflicts": True,
                "conflict_details": f"Unable to analyze for conflicts due to parsing error: {str(e)}. Flagging for manual review.",
                "severity": "medium",
                "safe_to_proceed": False,
                "conflicts": ["Conflict detection analysis failed - requires manual review"]
            }
            ...
            error_result = {
                "has_conflicts": True,
                "conflict_details": f"Conflict detection failed: {str(e)}. Requires manual review for safety.",
                "severity": "high",
                "safe_to_proceed": False,
                "conflicts": [f"System error during conflict detection: {str(e)}"]
            }
            return json.dumps(error_result)
